...
- **So is the third one.** `Tally` is a rendered character rather than an SVG:
  a small floating pod with a visor, a crest that droops when things are going
  badly, and two unattached mitts. It is not downloaded artwork — every vertex,
  material and animation clip is generated by `scripts/make-character-model.mjs`,
  which is in this repo, so its provenance is readable rather than asserted. Its
  six poses are clips on one model, which is what makes them the same character
  in six states by construction rather than by six drawings agreeing with each
  other. Pick it from the sparkle icon in the nav.
